 ...
 * @since JavaFX 2.0
 */
public abstract class AnimationTimer_improved implements TimerReceiver {
    ...
    private class AnimationTimerReceiver implements TimerReceiver {
        @Override public void handle(final long now) {
            ...
            AccessController.doPrivileged((PrivilegedAction<Void>) () -> {
                AnimationTimer_improved.this.handle(now);
                return null;
            }, accessCtrlCtx);
        }
    }
     ...
     * Creates a new timer.
     */
    public AnimationTimer_improved() {
        timer = Toolkit.getToolkit().getMasterTimer();
    }

    // For testing only
    AnimationTimer_improved(AbstractMasterTimer timer) {
        this.timer = timer;
    }
